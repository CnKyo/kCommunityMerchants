...
- (void)setupTitleBar {
    ...
    titleView.backgroundColor = [UIColor colorWithRed:1.000 green:0.184 blue:0.314 alpha:1];
...
- (void)setupWeekHeader {
    ...
    for (int i = 0; i < count; i++) {
        ...
        if (i == 0 || i == count - 1) {
            weekdayLabel.textColor = [UIColor colorWithRed:1.000 green:0.184 blue:0.314 alpha:1];
        } else {
...
- (void)calendarItem:(FDCalendarItem *)item didSelectedDate:(NSDate *)date {
    self.date = date;
    [self setCurrentDate:self.date];
    [self.delegate calendar:item didSelectedDate:date];
}
